...
export const TileShow=({title,desc,isComplete,id,toggleInfo,showInfo})=>{

    return (
        <View style={TileStyle.Outercontainer}>
            <Pressable style={TileStyle.container} onPress={()=>{showInfo(id)}}>
            <View style={TileStyle.container} >
                <Text style={TileStyle.title}>{title}</Text>
                <Text style={TileStyle.desc}>{desc?.length>12?desc.substring(0,12)+" ...":desc}</Text>
            </View>
            </Pressable>
            ...
            <View style={TileStyle.deleteIcon}>
                   {
                    isComplete
                    ?
                    <Icon name="heart" size={30} style={TileStyle.iconSelected} onPress={()=>toggleInfo(id)} />
                  :
                  <Icon name="heart" size={30} style={TileStyle.icon} onPress={()=>toggleInfo(id)}  />
                   } 
            </View>
        </View>
    )
